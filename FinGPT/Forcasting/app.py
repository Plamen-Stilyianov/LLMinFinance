...
q_config = bnb_config = BitsAndBytesConfig( load_in_8bit=True,
...
#for k in tokenizer.keys():
#    tokenizer[k] = tokenizer[k].cuda()
    ...
    for date in steps[:-1]:
        for i in range(len(stock_data)):
            if available_dates[i] >= date:
                prices.append(stock_data.iloc[i]['Close'])
                dates.append(datetime.strptime(available_dates[i], "%Y-%m-%d"))
                break

    dates.append(datetime.strptime(available_dates[-1], "%Y-%m-%d"))
    prices.append(stock_data.iloc[-1]['Close'])
...
#         print(symbol, ': ', start_date, ' - ', end_date)
        ...
        weekly_news_pre = finnhub_client.company_news(symbol, _from=start_date, to=end_date)
        if len(weekly_news_pre) == 0:
        ...
        weekly_news_pre = [
            {
                ...
                "headline": n['headline'],
                "summary": n['summary'],
            } for n in weekly_news_pre
        ]
        weekly_news = [news for news in weekly_news_pre if news['date'] > 0]
        for news in weekly_news:
            news['date'] = datetime.fromtimestamp(news['date']).strftime('%Y%m%d%H%M%S')
...
def get_prompt_by_row(symbol, row):
    ...
    term = 'increased' if float(row['End Price'][symbol]) > float(row['Start Price'][symbol]) else 'decreased'
    head = "From {} to {}, {}'s stock price {} from {:.2f} to {:.2f}. Company news during this period are listed below:\n\n".format(
        start_date, end_date, symbol, term, float(row['Start Price'][symbol]), float(row['End Price'][symbol]))
